...
def sheet_filter_queryset(serializer,user_id,parameter):
    ...
    if server_id=='' or server_id=="":
        server_id=None
    elif file_id=='' or file_id=="":
        file_id=None
...
def sheet_s_u(serializer,u_id,sh_id,parameter):
    ...
    if parameter=="save":
        if file_id==None or file_id=='':
            if models.sheet_data.objects.filter(user_id=u_id,queryset_id=queryset_id,server_id=server_id,sheet_name=sheet_name).exists() or models.sheet_data.objects.filter(queryset_id=queryset_id,server_id=server_id,user_ids__contains=u_id,sheet_name=sheet_name).exists():
                ...
                sh123=models.sheet_data.objects.create(user_id=u_id,chart_id=chart_id,queryset_id=queryset_id,server_id=server_id,filter_ids=filterId,sheet_filt_id=sheet_fil_qid,
            ...
            if models.sheet_data.objects.filter(user_id=u_id,queryset_id=queryset_id,file_id=file_id,sheet_name=sheet_name).exists() or models.sheet_data.objects.filter(queryset_id=queryset_id,server_id=server_id,user_ids__contains=u_id,sheet_name=sheet_name).exists():
                return Response({'message':'Sheet Name already exists, please rename the sheet name'},status=status.HTTP_406_NOT_ACCEPTABLE)
            else:
                dl_key=""
                file_sv=file_save_1(data,file_id,queryset_id,ip='sheetdata',dl_key=dl_key)
                sheet_fil_qid=sheet_filter_queryset(serializer,u_id,parameter='SAVE')
                sh123=models.sheet_data.objects.create(user_id=u_id,chart_id=chart_id,queryset_id=queryset_id,file_id=file_id,filter_ids=filterId,sheet_filt_id=sheet_fil_qid,
                                                    datapath=file_sv["file_key"],datasrc=file_sv["file_url"],created_at=created_at,updated_at=updated_at,
                                                    sheet_name=sheet_name,sheet_tag_name=sheet_tag_name)
                models.sheet_data.objects.filter(id=sh123.id).update(created_at=created_at,updated_at=updated_at)
            return Response({"sheet_id":sh123.id,"sheet_name":sh123.sheet_name,'message':'Saved Successfully'},status=status.HTTP_200_OK)
        ...
        if file_id==None or file_id=='':
            if models.sheet_data.objects.filter(queryset_id=queryset_id,server_id=server_id,id=sh_id).exists(): # user_id=u_id,
            # or models.sheet_data.objects.filter(user_ids__contains=u_id,queryset_id=queryset_id,server_id=server_id,id=sh_id).exists():
                ...
                if old.sheet_name==sheet_name:
                    ...
                    models.sheet_data.objects.filter(queryset_id=queryset_id,server_id=server_id,id=sh_id).update(
                        chart_id=chart_id,datapath=file_sv["file_key"],datasrc=file_sv["file_url"],updated_at=updated_at,filter_ids=filterId,
                        sheet_filt_id=sheet_fil_qid,sheet_tag_name=sheet_tag_name)
                    models.sheet_data.objects.filter(id=sh_id).update(updated_at=updated_at)
                    return Response({'message':'Updated Successfully'},status=status.HTTP_200_OK)
                else:
                    if models.sheet_data.objects.filter(queryset_id=queryset_id,server_id=server_id,sheet_name=sheet_name).exists(): #user_id=u_id,
                        ...
                        file_sv=file_save_1(data,server_id,queryset_id,ip='sheetdata',dl_key=old.datapath)
                        models.sheet_data.objects.filter(queryset_id=queryset_id,server_id=server_id,id=sh_id).update(sheet_name=sheet_name,filter_ids=filterId,
            ...
            if models.sheet_data.objects.filter(queryset_id=queryset_id,file_id=file_id,id=sh_id).exists(): #user_id=u_id,
                # or models.sheet_data.objects.filter(user_ids__contains=u_id,queryset_id=queryset_id,file_id=file_id,id=sh_id).exists():
                old= models.sheet_data.objects.get(id=sh_id)
                sheet_fil_qid=sheet_filter_queryset(serializer,u_id,parameter='UPDATE')
                if old.sheet_name==sheet_name:
                    file_sv=file_save_1(data,file_id,queryset_id,ip='sheetdata',dl_key=old.datapath)
                    models.sheet_data.objects.filter(queryset_id=queryset_id,file_id=file_id,id=sh_id).update(
                        chart_id=chart_id,datapath=file_sv["file_key"],datasrc=file_sv["file_url"],updated_at=updated_at,filter_ids=filterId,
                        sheet_filt_id=sheet_fil_qid,sheet_tag_name=sheet_tag_name)
                    models.sheet_data.objects.filter(id=sh_id).update(updated_at=updated_at)
                    return Response({'message':'Updated Successfully'},status=status.HTTP_200_OK)
                else:
                    if models.sheet_data.objects.filter(queryset_id=queryset_id,file_id=file_id,sheet_name=sheet_name).exists(): #user_id=u_id,
                        # or models.sheet_data.objects.filter(user_id__contains=u_id,queryset_id=queryset_id,file_id=file_id,sheet_name=sheet_name).exists():
                        return Response({'message':'Sheet name already exists for this queryset, data and sheet name will not update'},status=status.HTTP_406_NOT_ACCEPTABLE)
                    else:
                        file_sv=file_save_1(data,file_id,queryset_id,ip='sheetdata',dl_key=old.datapath)
                        models.sheet_data.objects.filter(queryset_id=queryset_id,file_id=file_id,id=sh_id).update(sheet_name=sheet_name,filter_ids=filterId,
                            chart_id=chart_id,datapath=file_sv["file_key"],datasrc=file_sv["file_url"],updated_at=updated_at,sheet_filt_id=sheet_fil_qid,
                            sheet_tag_name=sheet_tag_name)
                        models.sheet_data.objects.filter(id=sh_id).update(updated_at=updated_at)
                        return Response({'message':'Updated Successfully'},status=status.HTTP_200_OK)
        ...
        if tok1['status']==200:
            ...
            if serializer.is_valid(raise_exception=True):  
                ...
                try:
                    if file_id==None or file_id=='':
                        try:
                            sheetdata=models.sheet_data.objects.get(queryset_id=queryset_id,server_id=server_id,id=shid)#user_id=tok1['user_id'],
                        except:
                            sheetdata=models.sheet_data.objects.get(queryset_id=queryset_id,server_id=server_id,id=shid)#user_ids__contains=tok1['user_id'],
                        surl=sheetdata.datasrc
                    else:
                        try:
                            sheetdata=models.sheet_data.objects.get(queryset_id=queryset_id,file_id=file_id,id=shid)#user_id=tok1['user_id'],
                        except:
                            sheetdata=models.sheet_data.objects.get(queryset_id=queryset_id,file_id=file_id,id=shid)#user_ids__contains=tok1['user_id'],
                        surl=sheetdata.datasrc
                except:
...
def dashboard_dt(charts,charts_user,tok1):
    ...
    for ch in charts_user:
        ...
        #         else:
        #             return Response({'message':sheetsdata['message']},status=sheetsdata['status'])
        created_by = "Example" if ch['is_sample'] else tok1['username']
        data = {
            "created_by":created_by,
            "sheet_ids":litera_eval(ch['sheet_ids']),
            "dashboard_id":ch['id'],
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                if file_id==None or file_id=='':
                    if search=='':
                        ...
                        charts_users = models.dashboard_data.objects.filter(user_ids__contains=[tok1['user_id']],server_id__contains=server_id,queryset_id__contains=queryset_id).values().order_by('-updated_at')
                    else:
                        ...
                        charts_users = models.dashboard_data.objects.filter(user_ids__contains=[tok1['user_id']],server_id__contains=server_id,queryset_id__contains=queryset_id,dashboard_name__icontains=search).values().order_by('-updated_at')
                else:
                    if search=='':
                        ...
                        charts_users = models.dashboard_data.objects.filter(user_ids__contains=[tok1['user_id']],file_id__contains=file_id,queryset_id__contains=queryset_id).values().order_by('-updated_at')
                    else:
                        ...
                        charts_users = models.dashboard_data.objects.filter(user_ids__contains=[tok1['user_id']],file_id__in=file_id,queryset_id__contains=queryset_id,dashboard_name__icontains=search).values().order_by('-updated_at')
    ...
    def post(self,request,token):
        ...
        if tok1['status']==200:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                sheet_list=[]
                if file_id==None or file_id=='':
                    sheet_dt=models.sheet_data.objects.filter(server_id=server_id,queryset_id=queryset_id).values('id','sheet_name').order_by('id')  ##,user_id=tok1['user_id']
                    ...
                    sheet_dt=models.sheet_data.objects.filter(file_id=file_id,queryset_id=queryset_id).values('id','sheet_name').order_by('id')  ##,user_id=tok1['user_id']
                    # sheet_dt2=models.sheet_data.objects.filter(file_id=file_id,queryset_id=queryset_id,user_ids__contains=tok1['user_id']).values('id','sheet_name')
                for sh in sheet_dt:
        ...
        if tok1['status']==200:
            ...
            for qr in qrsets:
                try:
                    ...
                    dsqr_id=None
                data.append({'id':qr['queryset_id'],'queryset_name':qr['query_name'],'server_id':qr['server_id'],'file_id':qr['file_id'],'datasource_queryset_id':dsqr_id})
            return Response(data,status=status.HTTP_200_OK)
        else:
            return Response(tok1,status=tok1['status'])
    else:
