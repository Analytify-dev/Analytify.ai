...
#                 if not parent_ids.objects.filter(id=qs.hierarchy_id).exists():
#                     return Response({'message': "Hierarchy ID Not Found for Queryset"}, status=status.HTTP_404_NOT_FOUND)
                
#                 qs = QuerySets.objects.get(queryset_id=query_set_id)
#                 parent = parent_ids.objects.get(id = qs.hierarchy_id)
                
#                 is_file =False
#                 if FileDetails.objects.filter(id=parent.table_id).exists():
#                     is_file = True
#                     file=FileDetails.objects.get(id=parent.table_id)
...
#                         if 'error' in final_res:
#                             return Response({"message": final_res['error']}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
#                 elif ServerDetails.objects.filter(id= parent.table_id):
#                     sd = ServerDetails.objects.get(id=parent.table_id)
...
def rewrite_datatypes_for_metadata(meta_data, API_KEY):
    ...
    message = [{'role': 'user', 'content': f"Rewrite the JSON format by change only mentioned Datatypes  VARCHAR, INTEGER , FLOAT,BOOLEAN and DATATYPE for column {meta_data}. Ensure the JSON structure is syntactically correct, does not contain duplicate or misplaced keys, and maintains proper nesting. RETURN ONLY the JSON output based on these rules. DO NOT include any explanations or additional text."}]
    try:
        API_KEY = API_KEY
        response = requests.post(
            "https://api.openai.com/v1/chat/completions",
...
def chart_suggestions_on_metadata(meta_data, prompt,API_KEY):
    if prompt is None:
        ...
        message = [
        {
            ...
            Ensure the JSON structure is syntactically correct, does not contain duplicate or misplaced keys, and maintains proper nesting.
    ...
    def post(self, request,token):
        ...
        if tok1['status']==200:
            ...
            if serializer.is_valid():   
                ...
                if FileDetails.objects.filter(id = parent.table_id).exists() and parent.parameter.lower() == 'files':
                    is_file = True
                    file=FileDetails.objects.get(id=parent.table_id)
                    # fi_type=FileType.objects.get(id=file.file_type)
                    ...
                    if qs.is_custom_sql:
                        ...
                        # if files_data['status']==200:
                        try:
                            clickhouse_class = clickhouse.Clickhouse(file.display_name)
                ...
                elif ServerDetails.objects.filter(id=parent.table_id) and parent.parameter.lower() == 'server':
                    sd = ServerDetails.objects.get(id = parent.table_id)
                    server_type = ServerType.objects.get(id=sd.server_type).server_type
                    ...
                    # cursor = server_conn['cursor']
                    # db_type = server_type
                elif HaloPs.objects.filter(id = parent.table_id) and parent.parameter.lower() == 'halops':
                    halo_psa=HaloPs.objects.get(id = parent.table_id)

                    try:
                        clickhouse_class = clickhouse.Clickhouse(halo_psa.display_name)
                        engine=clickhouse_class.engine
                        cursor=clickhouse_class.cursor
                    except:
                        return Response({'message':"Connection closed, try again"},status=status.HTTP_406_NOT_ACCEPTABLE)
                
                elif connectwise.objects.filter(id = parent.table_id) and parent.parameter.lower() == 'connectwise':
                    connect_wise=connectwise.objects.get(id = parent.table_id)

                    try:
                        clickhouse_class = clickhouse.Clickhouse(connect_wise.display_name)
                        engine=clickhouse_class.engine
                        cursor=clickhouse_class.cursor
                    except:
                        return Response({'message':"Connection closed, try again"},status=status.HTTP_406_NOT_ACCEPTABLE)
