...
def ids_final_status(hierarchy_id):
    ...
    return status,parameter,server_id,file_id,quickbooks_id,salesforce_id,halops_id,connectwise_id,pr_id
...
def classify_columns(column_wise_first_record,names, type,table_alias):
    ...
    integer_list=['numeric','int','float','number','double precision','smallint','integer','bigint','decimal','numeric','real','smallserial','serial','bigserial','binary_float','binary_double','int64','int32','float64','float32','nullable(int64)','nullable(int32)','nullable(uint8)','nullable(flaot(64))']
    char_list=['varchar','bp char','text','varchar2','NVchar2','long','char','Nchar','character varying','string','str','nullable(string)']
    bool_list=['bool','boolean']
    date_list=['date','time','datetime','timestamp','timestamp with time zone','timestamp without time zone','timezone','time zone','timestamptz','nullable(datetime)','date32','date64'] 
    date_list11=['date','timestamp without time zone'] 
    timestamp_list = ['time','datetime','timestamp','timestamp with time zone','timezone','time zone','timestamptz','datetime64','datetime32']
    dimensions = []
    measures = []
...
def custom_sql(search,column_wise_first_record,type_codes,column_list,ser_db_data,queryset_id,data_types,user_id,pr_id,quer_tb):
...
def joining_sql(column_wise_first_record,search,type_codes,quer_tb,queryset_id,ser_db_data,server_type,data_types,user_id,pr_id):
    # if parameter=="file":
    #     server_id1=None
    #     file_id=server_id
    # else:
    ...
    def post(self,request,token):
        ...
        if tok1['status']==200:
            ...
            if serializer.is_valid(raise_exception=True):  
                ...
                status1,parameter,server_details_id,file_id,quickbooks_id,salesforce_id,halops_id,connectwise_id,pr_id=ids_final_status(server_details_id1)
                if status1 != 200:
                ...
                # ser_db_data=Connections.display_name(server_details_id1)
                try:
                    ser_db_data=Connections.display_name(server_details_id1)
                    database_name=ser_db_data.display_name
                except:
                ...
                if parameter=='server':
                    ...
                    server_type=ser_data.server_type.upper()
                elif parameter=='files':
                    ser_data=models.FileType.objects.get(id=ser_db_data.file_type)
                    server_type=ser_data.file_type.upper()
                else:
                    server_type=None
                if server_details_id==None and file_id==None and quickbooks_id==None and salesforce_id==None and connectwise_id==None and halops_id==None:
                    return Response({'message':'None type of id is not acceptable'},status=status.HTTP_406_NOT_ACCEPTABLE)
                else:
                    pass
                database_name=ser_db_data.display_name
                try:
                ...
                if server_type=="MONGODB":
                    ...
                    return mongo
                elif server_type=="MICROSOFTSQLSERVER":
                ...
                if quer_tb.is_custom_sql==True:
                    custom=custom_sql(search,column_wise_first_record,type_codes,column_list,ser_db_data,queryset_id,data_types,tok1['user_id'],pr_id,quer_tb)
                    cursor.close()
                    return custom
                else:
                    joining=joining_sql(column_wise_first_record,search,type_codes,quer_tb,queryset_id,ser_db_data,server_type,data_types,tok1['user_id'],pr_id)
                    cursor.close()
                    return joining
            else:
